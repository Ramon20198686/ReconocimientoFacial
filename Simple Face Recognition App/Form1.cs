...
namespace Simple_Face_Recognition_App
{
    public partial class Form1 : Form
    {
        ...
        CascadeClassifier faceCasacdeClassifier = new CascadeClassifier(@"C:\Users\User\source\repos\ReconocimientoFacial\Simple Face Recognition App\haarcascade_frontalface_alt.xml");
                            ...
                            if (EnableSaveImage)
                            {
                                ...
                                Task.Factory.StartNew(() => {
                                    for (int i = 0; i < 1; i++)
                                    {
                                        ...
                                        //if (txtPersonName.Text.Trim().Equals("") || txtPersonName.Text.Trim().Equals(""))
                                        //{
                                        ...
                                        //myStream.CopyTo(ms);
                                        file = ms.ToArray();
                                        file = resultImage.Bytes;//
                                        }

                                        using (Model.REC_FACIALEntities db = new Model.REC_FACIALEntities())
                                        {
                                                    Model.IMAGENES oImage = new Model.IMAGENES();
                                                    oImage.NOMBRE = txtPersonName.Text.Trim();
                                            oImage.IMAGEN = file;//
        ...
        private void btnCargarImgen_Click(object sender, EventArgs e)
        {
            ...
            {
                txtPersonName.Text = openFileDialog1.FileName;
            }
        }
